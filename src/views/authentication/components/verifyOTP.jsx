  ...
  return (
    <>
      <Box sx={{ textAlign: 'center', padding: 2, marginBottom: 2 }}>
        ...
        <Typography variant="subtitle1">00:{seconds > 9 ? seconds : '0' + seconds}</Typography>
      </Box>

      <OTPCodes optcode={(code) => EnteredCode(code)} />
      <Typography variant="subtitle2" color={theme.palette.error.light} sx={{ marginTop: 3 }}>
        {prompt}
      </Typography>
      <Button
        variant="contained"
        ...
        onClick={() => VerifyCode()}
      >
        {isSubmitting ? <CircularProgress size={22} sx={{ color: theme.palette.background.default }} /> : 'Verify'}
      </Button>

      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingY: 1 }}>
        <Typography variant="subtitle1" sx={{ cursor: 'pointer', color: theme.palette.grey[500] }} onClick={onWrong}>
          Wrong number
        </Typography>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingY: 2 }}>
          <Typography variant="subtitle2">Didn't get the code?</Typography>
          <Button variant="text" color="primary" sx={{ marginLeft: 0.3 }} disabled={resend} onClick={onResend}>
            {isResending ? <CircularProgress size={18} sx={{ color: theme.palette.primary.main }} /> : 'Resend'}
          </Button>
        </Box>
      </Box>
      <ToastContainer />
