...
const CreateTask = ({ open, handleCloseModal, kpi, handleTaskSubmission, submitting }) => {
  ...
  return (
    ...
    >
      <FormControl sx={{ border: 'none', boxShadow: 'none', p: 0 }} fullWidth>
        ...
        <Select
          ...
          name="plan_id"
          label="Select KPI"
          value={formik.plan_id}
          onChange={formik.handleChange}
          inputProps={{ 'aria-label': 'kpi' }}
          fullWidth
        >
          {kpi.map((plan, index) => (
      ...
      <FormControl fullWidth error={formik.touched.task && Boolean(formik.errors.task)} sx={{ marginTop: 3 }}>
        ...
        <OutlinedInput id="task" name="task" label="Task name" value={formik.values.task} onChange={formik.handleChange} fullWidth />
