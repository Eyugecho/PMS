...
const StaticPeriodsComponent = ({ data, fiscalYear, onRefresh }) => {
  ...
  const hasChanges = (key) => {
    ...
    return originalItem?.start_date !== currentItem?.start_date || originalItem?.end_date !== currentItem?.end_date;
  };
  ...
  const shouldDisableDate = (date, isStartDate, key) => {
    const startDate = periods[key]?.start_date ? new Date(periods[key].start_date) : null;

    if (isStartDate) {
      return isBefore(date, fiscalYear?.start_date) || isAfter(date, fiscalYear?.end_date);
    } else {
      return (
