...
const Conversations = ({ taskID, status }) => {
  ...
  const handleGettingRemarks = async () => {
    ...
    const Api = Backend.api + Backend.getApprovalTaskRemarks + taskID + `?page=${remarks.page + 1}&per_page=${remarks.per_page}`;
  ...
  useEffect(() => {
    if (mounted) {
      taskID && handleGettingRemarks();
    } else {
      setMounted(true);
    }
  }, [remarks.page, remarks.per_page, status]);
