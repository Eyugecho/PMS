    ...
    <DrogaCard sx={{ padding: 3 }}>
      ...
      <form>
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          ...
          {profileImage && (
            <Button variant="text" sx={{ marginLeft: 2, borderRadius: 2 }} onClick={() => handleRemovingProfile()}>
              Remove
            </Button>
          )}
        </Box>
        {uploadProgress > 0 && <LinearProgress value={uploadProgress} sx={{ marginBottom: 2, marginLeft: 1 }} />}

        <input type="file" id="profileImageUpload" accept="image/*" style={{ display: 'none' }} onChange={handleImageChange} />
      </form>

      {userInfo?.name && <InfoList label={'Full name'} info={userInfo?.name} />}
      {userInfo?.email && <InfoList label={'Email'} info={userInfo?.email} />}
      {userInfo?.phone && <InfoList label={'Phone'} info={userInfo?.phone} />}

      <Grid container>
        <Grid item xs={12} md={6}>
          <InfoList label={'Unit'} info={'Product development'} />
        </Grid>
        <Grid item xs={12} md={6}>
          <InfoList label={'Position'} info={'Senior software developer'} />
        </Grid>
      </Grid>

      <InfoList label={'Start date'} info={'July 17, 2024'} />
    </DrogaCard>
  );
};
